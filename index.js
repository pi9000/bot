const { fetchJson, fetchText, range, parseMarkdown, cekNik } = require('./lib/function')
const { Telegraf, Context } = require('telegraf')
//const cekNik = require('./nulz/cekNik');
...
bot.on('message', async (lol) => {
	try {
		...
		switch (command) {
			...
			// NuLz Tools //
			case 'cekipo':
				if (args.length == 0) return await reply(`Example: ${prefix + command} https://google.com/ or ${prefix + command} google.com`)
					domainUrl = args[0]
					domain = domainUrl.replace(/^https?:\/\//, '').replace(/\/$/, '')
					apiCheck = await fetchText(`https://check.skiddle.id/?domain=${domain}`)
					result = apiCheck
					text = `[Cek Domain Internet Positif]
				DOMAIN: ${domain}
				STATUS: ${result}
					`
					await reply(text)
			...
			case 'cekip':
				if (args.length == 0) return await reply(`Example: ${prefix + command} https://1.1.1.1/ or ${prefix + command} 1.1.1.1`)
					ipaddress = args[0]
					cleanIP = ipaddress.replace(/^https?:\/\//, '').replace(/\/$/, '')
					loopupAPIKEY = 'db0afa8d972e38c6a17722023af2ca82793e0e88812580664a964a6b'
					loopupAPI = `https://api.ipdata.co/${cleanIP}?api-key=${loopupAPIKEY}`
					getIpData = await fetchJson(loopupAPI)
					ipData = getIpData
					ipFlag = ipData.flag
					ipFlagCaption = `NEGARA ${ipData.country_name}`
					mess = `
					IP ADDRESS: ${cleanIP}
					IS_TOR: ${ipData.threat.is_tor}
					IS_PROXY: ${ipData.threat.is_proxy}
					IS_ANONYMOUS: ${ipData.threat.is_anonymous}
					CITY: ${ipData.city}
					REGION: ${ipData.region}
					REGION_CODE: ${ipData.region_code}
					REGION_TYPE: ${ipData.region_type}
					COUNTRY: ${ipData.country_name}
					COUNTRY_CODE: ${ipData.country_code}
					CONTINENT: ${ipData.continent_name}
					CONTINENT_CODE: ${ipData.continent_code}
					POSTAL_CODE: ${ipData.postal}
					LANGUAGES: ${ipData.languages[0].name}
					LANGUAGES_CODE: ${ipData.languages[0].code}
					CURRENCY: ${ipData.currency.name}
					CURRENCY_CODE: ${ipData.currency.code}
					CURRENCY_SYMBOL: ${ipData.currency.symbol}
					CURRENCY_PLURAL: ${ipData.currency.plural}
					LOCATION: https://www.google.com/maps?q=${ipData.latitude},${ipData.longitude}
					ASN (
						ASN: ${ipData.asn.asn}
						ASN_NAME: ${ipData.asn.name}
						ASN_DOMAIN: ${ipData.asn.domain}
						ASN_ROUTE: ${ipData.asn.route}
						ASN_TYPE: ${ipData.asn.type}
					)
					TIME_ZONE (
						NAME: ${ipData.time_zone.name}
						ABBR: ${ipData.time_zone.abbr}
						CURRENT_TIME: ${ipData.time_zone.current_time}
					)
					`
					await lol.replyWithPhoto({url: ipFlag}, {caption: ipFlagCaption, parse_mode: 'Markdown'})
					await reply(mess)
				break

			case 'ceknik':
				if (args.length == 0) return await reply(`Example: ${prefix + command} 123456789000000`)
					nik = args[0].toString()
					cleanedNik = nik.replace(/\D/g, '')
					niknya = cleanedNik
					if (/^\d{16}$/.test(niknya)) {
						result = cekNik(niknya)
						await reply(result)
					} else {
						await reply('Nomor NIK tidak valid. Pastikan nomor NIK terdiri dari 16 digit angka.');
					}
				break

			// Sans //
			case 'artinama':
				if (args.length == 0) return await reply(`Example: ${prefix + command} NuLz`)
					if (args.length > 1) {
						namaNya = args.join(" ")
						generateGambar = `https://api.lolhuman.xyz/api/ttp?apikey=${apikey}&text=${namaNya}`
						cekNama = await fetchJson(`https://api.lolhuman.xyz/api/artinama?apikey=${apikey}&nama=${namaNya}`)
						dataNama = cekNama.result
						await lol.replyWithPhoto({url: generateGambar})
						await reply(dataNama)
						...
						await reply(`Example: ${prefix + command} NuLz`)
					}
				break
			...
			case 'attp':
				if (args.length == 0) return await reply(`Example: ${prefix + command} NuLz`)
					if (args.length > 1) {
						textnya = args.join(" ")
						attpAPI = `https://api.lolhuman.xyz/api/attp?apikey=${apikey}&text=${textnya}`
						await lol.replyWithSticker({url: attpAPI})
					} else if (args.length == 1) {
						textnya = args[0]
						attpAPI = `https://api.lolhuman.xyz/api/attp?apikey=${apikey}&text=${textnya}`
						await lol.replyWithSticker({url: attpAPI})
			...
			case 'ytmp3':
				...
				result = await fetchJson(`https://api.lolhuman.xyz/api/ytaudio?apikey=${apikey}&url=${args[0]}`)
				result = result.result
				caption = `\`❖ Title    :\` *${result.title}*\n`
				caption += `\`❖ Size     :\` *${result.size}*`
				await lol.replyWithPhoto({ url: result.thumbnail }, { caption: caption, parse_mode: 'Markdown' })
				if (Number(result.size.split(` MB`)[0]) >= 50.0) return await reply(`Sorry the bot cannot send more than 50 MB!`)
				await lol.replyWithAudio({ url: result.link, filename: result.title })
				break
			case 'ytmp4':
				if (args.length == 0) return await reply(`Example: ${prefix + command} https://www.youtube.com/watch?v=qZIQAk-BUEc`)
				result = await fetchJson(`https://api.lolhuman.xyz/api/ytvideo?apikey=${apikey}&url=${args[0]}`)
				result = result.result
				caption = `\`❖ Title    :\` *${result.title}*\n`
				caption += `\`❖ Size     :\` *${result.size}*`
				await lol.replyWithPhoto({ url: result.thumbnail }, { caption: caption, parse_mode: 'Markdown' })
